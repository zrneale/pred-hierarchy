...
library(tidyverse)
#load the raw data
data <- read.csv("Data_for_r1.csv",header=T)%>%
...
predmassdf <- read.csv("Pred\ Dry\ Mass.csv", header = T)
...
data <- predmassdf%>%
  ...
  mutate_at(vars(c(Trial, Bath, Location)), funs(factor))%>%
  right_join(data, by = c("Trial", "Bath", "Location"))
...
#Plot with data
cont.data%>%
  mutate(predicted = predict(cont.glmer, newdata = cont.data, 
...
cont.data <- cont.data%>%
  mutate(backmortProp = 1 - predict(cont.glmer, newdata = cont.data,
                                    type = "response", re.form = ~0),
...
feed.data <-cont.data%>%
  ...
  right_join(data, by = c("Trial","Bath"), keep = F)%>%
  mutate(Numeaten = Dead-backmortNum, Temp2C = (Temp2 - 32)*5/9)
...
All.glmer <- feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glmer(cbind(round(Numeaten), round(100 - Numeaten)) ~ Temp2C + Pred + I(Temp2C^2) + Pred*Temp2C + Pred*I(Temp2C^2) +
...
All.glm <- feed.data%>%
  filter(Pred != "Control")%>%
  na.omit()%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Temp2C + Pred:I(Temp2C^2) +  Pred + Pred:Predmass, family = "binomial", data = ., na.action = "na.pass")
...
Model1 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Temp2C + I(Temp2C^2) +  Pred + Predmass, family = "binomial", data = .)
Model2 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Temp2C + I(Temp2C^2) +  Pred + Predmass, family = "binomial", data = .)
Model3 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Temp2C + Pred:I(Temp2C^2) +  Pred + Predmass, family = "binomial", data = .)

#This one has lowest AIC
Model4 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Temp2C + Pred:I(Temp2C^2) +  Pred + Pred:Predmass, family = "binomial", data = .)


Model5 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Temp2C + I(Temp2C^2) +  Pred + Pred:Predmass, family = "binomial", data = .)
Model6 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Temp2C + I(Temp2C^2) +  Pred + Pred:Predmass, family = "binomial", data = .)
Model7 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Predmass:Temp2C + Predmass:I(Temp2C^2) +  Pred + Predmass, family = "binomial", data = .)
Model8 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Predmass:Temp2C + Predmass:I(Temp2C^2) +  Pred + Predmass, family = "binomial", data = .)
...
Model9 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Predmass:Temp2C + Predmass:I(Temp2C^2) +  Pred + Pred:Predmass + Predmass, family = "binomial", data = .)
Model10 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Predmass:Temp2C + I(Temp2C^2) +  Pred + Predmass + Predmass:Pred, family = "binomial", data = .)
Model11 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Predmass:Temp2C + I(Temp2C^2) +  Pred + Predmass, family = "binomial", data = .)
Model12 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Predmass:Temp2C + Pred:Predmass:I(Temp2C^2) +  Pred + Predmass, family = "binomial", data = .)
Model13 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Predmass:Temp2C + Pred:Predmass:I(Temp2C^2) +  Pred + Predmass + Pred:Predmass, family = "binomial", data = .)

Model14 = feed.data%>%
  filter(Pred != "Control")%>%
  drop_na(Numeaten)%>%
  glm(cbind(round(Numeaten), round(100 - Numeaten)) ~ Pred:Predmass:Temp2C + Pred:Predmass:I(Temp2C^2) +  Pred + Predmass:Pred, family = "binomial", data = .)


feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Irrorata")%>%
  ggplot(aes(x = Temp2C, y = Numeaten)) +
...
feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Irrorata")
  plot(x = filter(drop_na(feed.data), Pred == "Irrorata")$Temp2C, y = predict(All.glm, newdata = drop_na(filter(feed.data, Pred == "Irrorata"))))
```
...
runglmer <- function(predator, terms){
  feed.data%>%
    ...
    glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, terms)  + scale(Predmass) + (1|Trial) + (1|Bath), family = "binomial", data = ., control = glmerControl(optimizer = "bobyqa"))%>%
    return()
...
runglm <- function(species, terms){
  feed.data%>%
    na.omit()%>%
    filter(Pred == species)%>%
    glm(data = .,cbind(round(Numeaten), round(100 - Numeaten)) ~ poly(Temp2C, terms) + Predmass,
        family = "binomial")%>%
    return()
}
...
feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Buenoa")%>%
  ggplot(aes(x = Temp2C, y = Numeaten, color = Location)) + 
  #geom_smooth() +
  geom_point() +
  geom_jitter(width = 1)
...
feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Pachy")%>%
  glmer(data = ., cbind(round(Numeaten), round(100 - Numeaten)) ~ poly(Temp2C) +
...
model1 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Trial) + (1|Bath) + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model2 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Trial) + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model3 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Trial), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model4 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Trial)  + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model5 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model6 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model7 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2) + (1|Trial) + (1|Bath) + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model8 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2) + (1|Trial) + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model9 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2) + (1|Trial), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model10 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2) + (1|Trial)  + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model11 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2) + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model12 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2) + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model13<- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C  + scale(Predmass) + (1|Trial) + (1|Bath) + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model14 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C  + scale(Predmass) + (1|Trial) + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model15 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C  + scale(Predmass) + (1|Trial), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model16 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C  + scale(Predmass) + (1|Trial)  + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model17 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C  + scale(Predmass) + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model18 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C  + scale(Predmass) + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model19 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C + (1|Trial) + (1|Bath) + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model20 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C + (1|Trial) + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model21 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C + (1|Trial), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model22 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C + (1|Trial)  + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model23 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C + (1|Location), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))

model24 <- glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C + (1|Bath), family = "binomial", data = df, control = glmerControl(optimizer = "bobyqa"))
...
Buen.glmer <- feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Buenoa")%>%
  glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ Temp2C  + scale(Predmass) + (1|Trial) + (1|Bath), family = "binomial", data = ., control = glmerControl(optimizer = "bobyqa"))

Ind.glmer <- feed.data%>%
   drop_na(Numeaten)%>%
  filter(Pred == "Indica")%>%
  glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Trial) + (1|Location), family = "binomial", 
        data = ., control = glmerControl(optimizer = "bobyqa"))

Irr.glmer <- feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Irrorata")%>%
  glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2) + scale(Predmass) + (1|Trial) + (1|Bath) + (1|Location), family = "binomial", 
        data = ., control = glmerControl(optimizer = "bobyqa"))
...
Tram.glmer <- feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Tramea")%>%
  glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  + scale(Predmass) + (1|Bath), family = "binomial", 
        data = ., control = glmerControl(optimizer = "bobyqa"))
```
...
#Faceted by species
final.data%>%
  ggplot(aes(x = Temp2C, y = fit*100, color = Pred, fill = Pred)) +
...
Indata <- feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Indica")%>%
  mutate(Temp2C2 = Temp2C^2, scaledmass = scale(Predmass),
...
Ind.glmer.test <-feed.data%>%
  drop_na(Numeaten)%>%
  filter(Pred == "Indica")%>%
  glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C,2)  + scale(Predmass) + (1|Trial) + (1|Bath) + (1|Location), family = "binomial", data = ., control = glmerControl(optimizer = "bobyqa"))
...
feed.data%>%
  ...
  ggplot(aes(x = Temp2C, y = Numeaten)) +
  geom_point() +
  geom_line(aes(x = Temp2C, y = fit * 100))
...
lm(logit(Numeaten/50) ~ Temp2C, Indata)
...
nls(Numeaten ~ I(k / 1 + exp(-r * (Temp2C - a))), data = Indata,
...
nlsLM(Numeaten ~ I(k / 1 + exp(-r * (Temp2C - a))) + (1|Trial), data = Indata,
      start = list(k = log(50), r = 0.03, a = -5.7))
...
nls(Numeaten ~ SSasymp(Temp2C, Asym, R0, lrc), Indata)
...
Buen.glmer <- feed.data%>%
    drop_na(Numeaten)%>%
    filter(Pred == "Buenoa")%>%
    glmer(cbind(round(Numeaten), round(100-Numeaten)) ~ poly(Temp2C, 2)  +  scale(Predmass) + (1|Trial) + (1|Bath), family = "binomial", data = ., control = glmerControl(optimizer = "bobyqa"))
...
predictions(Buen.glmer, type = "response", variables = "Temp2C")
ggplot(aes(x = Temp2C))

#Trying the built in plotting function
plot_cap(Buen.glmer, condition= "Temp2C", type = "response")
...
library(ggeffects)
ggpredict(Buen.glmer, terms = "Temp2C [all]")
```
...
Feed.fit.df <- data.frame(Trial = NA, Bath = NA, Location = NA,  Pred = NA, Temp2C = NA, Numeaten = NA, Predmass = NA, fit = NA, fit_link = NA, se_link = NA, fit_resp = NA, upr = NA, lwr = NA)
...
#na.omit()%>%
#filter(Pred == "Buenoa")%>%
#with(data = ., data_frame(Temp2C = seq(min(Temp2C), max(Temp2C),
length = 1000)))
